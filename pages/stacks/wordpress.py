...
def deploy_wordpress(modal, default_id=None, default_image=None, default_name=None, default_domain=None, default_port=None, default_envs=None):
        ...
        var_envs        = envs
        var_volumes     = [(var_name+"_wordpress","/var/www/html")]
...
@ui.page('/form/wordpress')
def form_wordpress() -> None:
    ...
    default_envs = """WORDPRESS_DB_HOST=mysql
WORDPRESS_DB_USER=mysql_usuario
WORDPRESS_DB_PASSWORD=mysql_usuario_senha
WORDPRESS_DB_NAME=banco_dados_usuario"""
        
    ui.query('textarea').style('line-height: 24px')

    ui.label('Formulário wordpress').classes('text-2xl').style('color:gray')
    
    container_fields = ui.row().classes('w-full')
    
    with container_fields:
        fields.get_fields(template_deploy='wordpress', default_image=default_image, default_envs=default_envs)