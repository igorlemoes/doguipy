...
def deploy_mongoexpress(modal, default_id=None, default_image=None, default_name=None, default_domain=None, default_port=None, default_envs=None):
        ...
        var_port        = default_port if default_port else 8081
        var_envs        = envs
        var_volumes     = []
...
@ui.page('/form/mongoexpress')
def form_mongoexpress() -> None:
    ...
    default_image = 'mongo-express:latest'
    
    default_envs = """ME_CONFIG_MONGODB_SERVER=mongo
ME_CONFIG_BASICAUTH_USERNAME=express_usuario
ME_CONFIG_BASICAUTH_PASSWORD=express_senha
ME_CONFIG_MONGODB_ADMINUSERNAME=mongo_usuario
ME_CONFIG_MONGODB_ADMINPASSWORD=mongo_senha"""
        
    ui.query('textarea').style('line-height: 24px')

    ui.label('Formulário PgAdmin').classes('text-2xl').style('color:gray')
    
    container_fields = ui.row().classes('w-full')
    
    with container_fields:
        fields.get_fields(template_deploy='mongoexpress', default_image=default_image, default_envs=default_envs)