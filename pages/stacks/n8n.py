...
def deploy_n8n(modal, default_id=None, default_image=None, default_name=None, default_domain=None, default_port=None, default_envs=None):
        ...
        var_port        = default_port if default_port else 5678
        var_envs        = envs
        var_volumes     = [(var_name+"_n8n_data","/home/node/.n8n")]
...
def form_n8n() -> None:
    ...
    ui.label('Formulário n8n').classes('text-2xl').style('color:gray')
    
    container_fields = ui.row().classes('w-full')
    
    with container_fields:
        fields.get_fields(template_deploy='n8n', default_image=default_image, default_envs=default_envs)