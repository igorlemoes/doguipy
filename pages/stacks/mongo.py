...
def deploy_mongo(modal, default_id=None, default_image=None, default_name=None, default_domain=None, default_port=None, default_envs=None):
        ...
        var_port        = default_port if default_port else 27017
        var_envs        = envs
        var_volumes     = [
            (var_name+"_mongo_data","/data/db"),
            (var_name+"_mongo_configdb_data","/data/configdb")
        ]
...
def form_mongo() -> None:
    ...
    default_envs = """MONGO_INITDB_ROOT_USERNAME=mongo_usuario
MONGO_INITDB_ROOT_PASSWORD=mongo_senha"""
        
    ui.query('textarea').style('line-height: 24px')

    ui.label('Formulário mongo').classes('text-2xl').style('color:gray')
    
    container_fields = ui.row().classes('w-full')
    
    with container_fields:
        fields.get_fields(template_deploy='mongo', default_image=default_image, default_envs=default_envs)