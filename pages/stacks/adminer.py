...
def deploy_adminer(modal, default_id=None, default_image=None, default_name=None, default_domain=None, default_port=None, default_envs=None):
...
@ui.page('/form/adminer')
def form_adminer() -> None:
    ...
    default_image = 'adminer:latest'
    
    default_envs = """"""
        
    ui.query('textarea').style('line-height: 24px')

    ui.label('Formulário adminer').classes('text-2xl').style('color:gray')
    
    container_fields = ui.row().classes('w-full')
    
    with container_fields:
        fields.get_fields(template_deploy='adminer', default_image=default_image, default_envs=default_envs)